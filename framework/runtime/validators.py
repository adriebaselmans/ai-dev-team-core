...
from spec_loader import phase_canonical_source, repo_root, transition_spec
...
DEVELOPMENT_REQUIRED_PATHS = [
    'framework/config/models.yaml',
    'framework/runtime/context_slices.yaml',
...
def validate_requirements() -> ValidationResult:
    ...
    if phase_canonical_source('requirements') != 'artifacts':
        messages.append("Requirements phase must declare canonical_source 'artifacts'.")
...
def validate_design() -> ValidationResult:
    ...
    if phase_canonical_source('architecture') != 'artifacts':
        messages.append("Architecture phase must declare canonical_source 'artifacts'.")
    return _validation_result(messages, 'Design is implementation-safe.')


def validate_development() -> ValidationResult:
    messages: list[str] = []
    if phase_canonical_source('development') != 'artifacts':
        messages.append("Development phase must declare canonical_source 'artifacts'.")
    for relative_path in DEVELOPMENT_REQUIRED_PATHS:
        full_path = repo_root() / relative_path
        if not full_path.exists():
            messages.append(f'Missing required development output: {relative_path}')
...
def validate_review() -> ValidationResult:
    ...
    if phase_canonical_source('review') != 'artifacts':
        messages.append("Review phase must declare canonical_source 'artifacts'.")
...
def validate_dod() -> ValidationResult:
    ...
    if phase_canonical_source('testing') != 'artifacts':
        messages.append("Testing phase must declare canonical_source 'artifacts'.")
