...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.work_item:
        coordination = dict(overrides.get("coordination", {})) if isinstance(overrides.get("coordination"), dict) else {}
        coordination["work_items"] = [{"id": f"item-{index + 1}", "description": item} for index, item in enumerate(args.work_item)]
        overrides["coordination"] = coordination
    ...
    final_state = Orchestrator(flow, build_default_agent_registry()).run(initial_state)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compatibility wrapper around the active team orchestrator.")
    sub = parser.add_subparsers(dest="command", required=True)

    run = sub.add_parser("run", help="Run the active flow-driven orchestrator and persist final state.")
